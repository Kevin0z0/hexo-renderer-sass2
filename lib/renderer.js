...
module.exports = function (data) {
    const opt = extend(this.config || {}, load(fs.readFileSync(path.join(process.cwd(), "/themes", this.config.theme, "_config.yml"), 'utf8')) || {})
    const config = getOptions(opt.sass)
    const filePath = data.path
    setFunctions(config, opt)
    return new Promise((resolve, reject) => {
        return sass.compileAsync(filePath, config)
        .then(result => {
            if(opt.sass.sourceMap) {
                const hash = md5(Buffer.from(JSON.stringify(result.sourceMap)))
                if(hash !== mapHash.get(filePath)){
                    mapHash.set(filePath, hash)
                    saveSouceMap(filePath, JSON.stringify(result.sourceMap))
                }
            }
            ...
            if(opt.sass.autoprefixer){
                ...
                return postcss([autoprefixer]).process(result.css).then(r=>{
                    returnCSS(opt, r.css, filePath, resolve)
                })
            }
            returnCSS(opt, result.css, filePath, resolve)
        })
        .catch(err => {
            console.log(err)
            reject(err)
